		...
		<div class="container" id="mvAll">
			...
			<div class="row" >
				<c:if test="${not empty mvAll}">
					<c:forEach var="mvAll" items="${mvAll }" varStatus="i">
								...
								<div class="pi-text">
									<c:if test="${not empty nowid }">
										<c:if test="${mvAll.gdchk == 1}">
											<a href="favoritechoice?mv_num=${mvAll.mv_num}&page=1" class="heart-icon"><i class="fa fa-heart"></i></a>
										</c:if>
										<c:if test="${mvAll.gdchk ==0 }">
											<a href="favoritechoice?mv_num=${mvAll.mv_num}&page=1" class="heart-icon"><i class="fa fa-heart-o"></i></a>
										</c:if>
									</c:if>
									<div class="pt-price">
									<p class="mv_name" style="font-weight: 700;">
										...
										<c:if test="${mvAll.mv_grade == '청소년 관람불가' }">
									...
									<ul>
										...
										<li><button class="btn btn-success"
												onclick="location.href='reserve?mv_name=${mvAll.mv_name}&mv_num=${mvAll.mv_num }'">예매</button></li>
										</c:if>
									</ul>
								</div>
							</div>
