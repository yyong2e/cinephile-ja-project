...
<body>
	...
	<section class="property-section spad">
		...
		<div class="container" id="mvAllR">
			...
			<div class="row" >
				<c:if test="${not empty mvAllR}">
					<c:forEach var="mvAllR" items="${mvAllR }" varStatus="i">
							...
							<div class="property-item">
								...
								<div class="pi-text">
									<c:if test="${not empty nowid }">
										<c:if test="${mvAllR.gdchk == 1}">
											<a href="favoritechoice?mv_num=${mvAllR.mv_num}&page=1" class="heart-icon"><i class="fa fa-heart"></i></a>
										</c:if>
										<c:if test="${mvAllR.gdchk ==0 }">
											<a href="favoritechoice?mv_num=${mvAllR.mv_num}&page=1" class="heart-icon"><i class="fa fa-heart-o"></i></a>
										</c:if>
									</c:if>
									<div class="pt-price">
									<p class="mv_name" style="font-weight: 700;">
										<c:if test="${mvAllR.mv_grade == '전체 관람가' }">
											<img alt="" src="${path}/img/moviegrade-all.png"
												style="width: 23px;">
										</c:if>
										<c:if test="${mvAllR.mv_grade == '12세 관람가' }">
											<img alt="" src="${path}/img/moviegrade-12.png"
												style="width: 23px;">
										</c:if>
										<c:if test="${mvAllR.mv_grade == '15세 관람가' }">
											<img alt="" src="${path}/img/moviegrade-15.png"
												style="width: 23px;">
										</c:if>
										<c:if test="${mvAllR.mv_grade == '19세 관람가' }">
									...
									<p>
										<i class="fa fa-video-camera"></i> ${mvAllR.mv_direct }
									</p>
									<p>
									...
									<ul>
										...
										<li><i class="fa fa-star"></i> ${mvAllR.mv_rate }</li>
										<li><i class="fa fa-film"></i> ${mvAllR.mv_genre }</li>
										<c:if test="${mvAllR.rechk == 1 }">
										<li><button class="btn btn-success"
												onclick="location.href='reserve?mv_name=${mvAllR.mv_name}'">예매</button></li>
										</c:if>
									</ul>
									<!-- 									<div class="pi-agent">
										<div class="pa-item">
